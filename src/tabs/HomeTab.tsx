...
import { Server, Layers, Activity, Users, ArrowRight, Plus, Play, X, Globe } from "lucide-react";
...
export default function HomeTab({ setActiveTab, openInstance, onLaunchWithServer, onLaunchWorld }: { setActiveTab: (tab: string) => void, openInstance?: (id: string) => void, onLaunchWithServer?: (instanceId: string, serverIp: string) => void, onLaunchWorld?: (instanceId: string, worldName: string) => void }) {
  ...
  useEffect(() => {
    ...
    loadServers();
    invoke("get_last_world").then((w: any) => setLastWorld(w && w.instance_id ? w : null)).catch(() => setLastWorld(null));
  }, [loadServers]);
    ...
    <div className="flex flex-col w-full max-w-6xl mx-auto h-full gap-8 pb-10">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {servers.map((srv, i) => (
            <div key={i} className="bg-jm-card border border-white/10 rounded-2xl p-4 flex flex-col gap-2">
              ...
              {srv.icon ? (
                <>
                  <img src={srv.icon} alt="" className="w-16 h-16 rounded-xl object-cover shadow-md shrink-0" style={{ imageRendering: "pixelated" }} onError={e => { (e.target as HTMLImageElement).style.display = "none"; (e.target as HTMLImageElement).nextElementSibling?.classList.remove("hidden"); }} />
                  <div className="hidden w-16 h-16 rounded-xl bg-black/50 flex items-center justify-center border border-white/20 shrink-0"><Server size={24} className="text-gray-500" /></div>
                </>
              ) : (
                <div className="w-16 h-16 rounded-xl bg-black/50 flex items-center justify-center border border-white/5 shrink-0"><Server size={24} className="text-gray-500" /></div>
              )}
              <div className="flex-grow min-w-0">
                <div className="flex justify-between items-start">
                  <h3 className="font-bold text-white text-lg truncate group-hover:text-jm-accent-light transition-colors">{srv.name}</h3>
                  <div className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider ${srv.online ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                    <Activity size={12} /> {srv.online ? 'Online' : 'Offline'}
                  </div>
                </div>
                <p className="text-sm text-gray-400 truncate">{srv.ip}</p>
                <p className="text-xs text-gray-500 truncate mt-1" dangerouslySetInnerHTML={{ __html: srv.motd }}></p>
                {srv.last_played > 0 && (
              ...
              {srv.last_instance_id && srv.last_instance_name && onLaunchWithServer && (
                <div className="flex items-center justify-between pt-1 border-t border-white/5">
                  <span className="text-[10px] text-gray-500">Сборка: {srv.last_instance_name}</span>
      ...
      {lastWorld && (
        <div>
          <h2 className="text-2xl font-bold text-jm-accent-light flex items-center gap-2 mb-4"><Globe /> Последний мир</h2>
          <div className="bg-jm-card border border-white/10 rounded-2xl p-4 flex flex-col gap-2">
            <div
              onClick={() => openInstance && openInstance(lastWorld.instance_id)}
              className="flex items-center gap-4 hover:border-jm-accent/50 transition-all cursor-pointer hover:-translate-y-1 hover:shadow-lg group min-w-0"
            >
              <div className="w-16 h-16 rounded-xl bg-black/50 flex items-center justify-center border border-white/5 shrink-0">
                <Globe size={28} className="text-jm-accent" />
              </div>
              <div className="flex-grow min-w-0">
                ...
                <p className="text-sm text-gray-400 truncate">Сборка: {lastWorld.instance_name}</p>
            ...
            {onLaunchWorld && (
              <div className="flex items-center justify-between pt-1 border-t border-white/5">
                <span className="text-[10px] text-gray-500">Сборка: {lastWorld.instance_name}</span>
      ...
      {myInstances.length > 0 && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-bold text-jm-accent-light flex items-center gap-2"><Layers /> Ваши сборки</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {myInstances.map((inst) => (
              <div key={inst.id} onClick={() => openInstance ? openInstance(inst.id) : setActiveTab("library")} className="bg-jm-card border border-white/10 rounded-2xl p-4 flex items-center gap-4 hover:border-jm-accent/50 transition-all cursor-pointer hover:-translate-y-1 hover:shadow-lg group">
                {inst.icon ? (
                  <img src={convertFileSrc(inst.icon)} alt="" className="w-12 h-12 rounded-xl object-cover border border-white/10" onError={e => { (e.target as HTMLImageElement).style.display = "none"; (e.target as HTMLImageElement).nextElementSibling?.classList.remove("hidden"); }} />
                ) : null}
                <div className={`w-12 h-12 rounded-xl bg-black/50 border border-white/20 flex items-center justify-center text-sm font-medium text-white/70 shrink-0 ${inst.icon ? "hidden" : ""}`}>{inst.name?.charAt(0)?.toUpperCase() || "?"}</div>
                <div className="min-w-0">
                  <h3 className="font-bold text-white truncate group-hover:text-jm-accent-light transition-colors">{inst.name}</h3>
                  <div className="flex gap-2 mt-0.5"><span className="text-xs text-gray-400 capitalize">{inst.loader}</span><span className="text-xs text-gray-500">{inst.game_version}</span></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-jm-accent-light flex items-center gap-2"><Layers /> Популярные сборки</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {recommendedPacks.map((pack) => (
            <div key={pack.project_id} className="bg-jm-card border border-white/10 rounded-2xl p-4 flex flex-col hover:border-jm-accent/50 transition-all hover:-translate-y-1 hover:shadow-xl group cursor-pointer" onClick={() => setActiveTab("discover")}>
              {pack.icon_url ? (
                <><img src={pack.icon_url} alt="" className="w-16 h-16 rounded-xl object-cover bg-black/50 border border-white/20 mb-3 shadow-md" onError={e => { (e.target as HTMLImageElement).style.display = "none"; (e.target as HTMLImageElement).nextElementSibling?.classList.remove("hidden"); }} /><div className="hidden w-16 h-16 rounded-xl bg-black/50 border border-white/20 flex items-center justify-center text-sm font-medium text-white/70 mb-3 shrink-0 self-start">{((pack?.title || "?") as string).split(/\s+/).map((w: string) => w[0]).join("").slice(0, 2).toUpperCase() || "?"}</div></>
              ) : (
                <div className="w-16 h-16 rounded-xl bg-black/50 border border-white/20 flex items-center justify-center text-sm font-medium text-white/70 mb-3 shrink-0 self-start">{((pack?.title || "?") as string).split(/\s+/).map((w: string) => w[0]).join("").slice(0, 2).toUpperCase() || "?"}</div>
              )}
              <h3 className="font-bold text-white text-lg truncate group-hover:text-jm-accent-light transition-colors">{pack.title}</h3>
              <p className="text-xs text-jm-accent truncate mb-2">от {pack.author}</p>
              <p className="text-sm text-gray-400 line-clamp-2">{pack.description}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setAddServerModal(false)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={e => e.stopPropagation()} className="bg-jm-card border border-jm-accent/30 rounded-2xl p-6 w-full max-w-md shadow-2xl">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setAddServerModal(false)} className="text-gray-400 hover:text-white p-1 rounded-lg hover:bg-white/10"><X size={20} /></button>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">IP или адрес</label>
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Название (необязательно)</label>
              ...
              <div className="flex gap-2 mt-6">
                ...
                <button onClick={async () => {
                  setAddServerError("");
                  try {
                    await invoke("add_recent_server", { ip: addServerIp, name: addServerName });
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setServerMenu(null)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={e => e.stopPropagation()} className="bg-jm-card border border-jm-accent/30 rounded-2xl p-6 w-full max-w-md shadow-2xl">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setServerMenu(null)} className="text-gray-400 hover:text-white p-1 rounded-lg hover:bg-white/10"><X size={20} /></button>
              </div>
              <div className="mb-4 p-4 rounded-xl bg-black/30 border border-white/5">
                ...
                <p className="text-xs text-gray-400 mb-1">{serverMenu.ip}</p>
                {serverMenu.motd && <p className="text-sm text-gray-300 mb-2" dangerouslySetInnerHTML={{ __html: serverMenu.motd }}></p>}
              ...
              <p className="text-sm text-gray-400 mb-4">Выберите сборку для входа:</p>
              <div className="flex flex-col gap-2 max-h-64 overflow-y-auto custom-scrollbar">
                ...
                {serverMenuInstances.length === 0 && <div className="text-gray-500 text-center py-6">Нет сборок. Создайте сборку во вкладке «Сборки».</div>}
              </div>
            </motion.div>
          </motion.div>
        )}
