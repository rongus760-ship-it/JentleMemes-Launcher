...
function CustomSelect({ label, value, options, onChange, disabled = false }: any) {
  ...
  return (
    <div className={`flex flex-col relative w-full ${disabled ? 'opacity-50 pointer-events-none' : ''}`} ref={selectRef}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-[100%] mt-2 w-full bg-[#0b110b] border border-white/10 rounded-xl z-50 max-h-60 overflow-y-auto custom-scrollbar shadow-2xl">
...
function ExportModal({ instanceId, instanceName, onClose, showToast }: { instanceId: string; instanceName: string; onClose: () => void; showToast: (msg: string) => void }) {
  const [format, setFormat] = useState<"zip" | "mrpack">("zip");
  ...
  useEffect(() => {
    invoke("list_instance_folders", { id: instanceId }).then((dirs: any) => {
      ...
      for (const d of (dirs || [])) {
        state[d] = ["mods", "config", "resourcepacks"].includes(d);
      }
      setFolders(state);
  ...
  const folderLabels: Record<string, string> = { mods: "Моды", config: "Конфигурация", resourcepacks: "Ресурспаки", shaderpacks: "Шейдеры", saves: "Миры", scripts: "Скрипты" };
      ...
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="bg-jm-card border border-white/10 p-8 rounded-3xl w-full max-w-md shadow-2xl">
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-3">
            <button onClick={() => setFormat("zip")} className={`flex-1 py-3 rounded-xl font-bold text-sm transition-colors border ${format === "zip" ? "bg-jm-accent text-black border-jm-accent" : "bg-white/5 text-gray-400 border-white/10 hover:border-white/30"}`}>.zip (бэкап)</button>
            <button onClick={() => setFormat("mrpack")} className={`flex-1 py-3 rounded-xl font-bold text-sm transition-colors border ${format === "mrpack" ? "bg-jm-accent text-black border-jm-accent" : "bg-white/5 text-gray-400 border-white/10 hover:border-white/30"}`}>.mrpack (Modrinth)</button>
          </div>
        </div>

        <div className="mb-6">
          <label className="text-sm text-gray-400 mb-2 block">Включить папки</label>
          <div className="grid grid-cols-2 gap-2">
            {Object.keys(folders).map(key => (
              <label key={key} className="flex items-center gap-2 cursor-pointer p-3 bg-black/30 rounded-xl border border-white/5 hover:border-white/20 transition-colors">
                ...
                <span className="text-white text-sm font-bold">{folderLabels[key] || key}</span>
              </label>
            ))}
          </div>
        </div>
...
export default function LibraryTab({ initialInstanceId, initialServerIp, initialWorldName, onInstanceOpened, onServerLaunchConsumed, onWorldLaunchConsumed, busyInstanceId = null, progress = { task_name: "", downloaded: 0, total: 0 } }: { initialInstanceId?: string, initialServerIp?: string, initialWorldName?: string, onInstanceOpened?: () => void, onServerLaunchConsumed?: () => void, onWorldLaunchConsumed?: () => void, busyInstanceId?: string | null, progress?: ProgressPayload } = {}) {
  ...
  async function handleCreate() {
    ...
    await invoke("create_instance", { name: newName, gameVersion: newVersion, loader: newLoader, loaderVersion: newLoader === "vanilla" ? "" : newLoaderVersion });
    setIsCreating(false); setNewName(""); loadData(); showToast("Сборка создана!");
  }
  ...
  if (selectedInstance) {
    ...
    return (
      <div className="flex flex-col w-full max-w-6xl mx-auto h-full animate-in fade-in slide-in-from-right-8 duration-300">
        ...
        <div className="flex items-center justify-between mb-6 bg-jm-card p-6 rounded-3xl border border-white/10 shadow-xl shrink-0">
          <div className="flex items-center gap-6">
            <button onClick={() => setSelectedInstance(null)} className="p-3 bg-black/50 hover:bg-jm-accent hover:text-black text-white rounded-xl transition-colors">
              <ArrowLeft size={24} />
            </button>
            <div className="w-20 h-20 rounded-2xl overflow-hidden shrink-0 bg-black/50 flex items-center justify-center border border-white/20">
              {(() => { const iconSrc = selectedInstance.icon || packSourceInfo?.icon_url; return iconSrc ? <><img src={iconSrc.startsWith("http") ? iconSrc : convertFileSrc(iconSrc)} alt="" className="w-full h-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none"; (e.target as HTMLImageElement).nextElementSibling?.classList.remove("hidden"); }} /><span className="hidden w-full h-full flex items-center justify-center text-lg font-semibold text-white/70 border border-white/20">{selectedInstance.name?.charAt(0)?.toUpperCase() || "?"}</span></> : <span className="w-full h-full flex items-center justify-center text-lg font-semibold text-white/70 border border-white/20">{selectedInstance.name?.charAt(0)?.toUpperCase() || "?"}</span>; })()}
            </div>
            <div>
              <h2 className="text-3xl font-bold text-white mb-1">{selectedInstance.name}</h2>
              <div className="flex gap-2 text-sm">
                <span className="bg-white/10 px-3 py-1 rounded-lg text-gray-300 capitalize">{selectedInstance.loader}</span>
                <span className="bg-white/10 px-3 py-1 rounded-lg text-gray-300">{selectedInstance.game_version}</span>
                {selectedInstance.loader_version && <span className="bg-jm-accent/20 text-jm-accent-light px-3 py-1 rounded-lg border border-jm-accent/30">{selectedInstance.loader_version}</span>}
              </div>
            </div>
          </div>
          
          <div className="flex gap-3">
            <button onClick={checkPackUpdate} disabled={isCheckingPackUpdate} className="bg-white/10 hover:bg-white/20 text-white px-4 py-3 rounded-xl font-bold transition-colors flex items-center gap-2 disabled:opacity-50" title="Проверить обновления сборки">
              <RefreshCw size={18} className={isCheckingPackUpdate ? "animate-spin" : ""} /> Обновления
            </button>
            {packUpdateInfo?.has_update && packUpdateInfo?.update_url && (
              <button onClick={applyPackUpdate} disabled={isUpdatingPack} className="bg-blue-500 hover:bg-blue-400 text-white px-6 py-3 rounded-xl font-bold transition-colors flex items-center gap-2 disabled:opacity-50">
                {isUpdatingPack ? <Loader2 size={18} className="animate-spin" /> : <Download size={18} />} Обновить сборку
              </button>
            )}
            <button onClick={() => setShowExportModal(true)} className="bg-white/10 hover:bg-white/20 text-white px-6 py-3 rounded-xl font-bold transition-colors flex items-center gap-2"><Archive size={18}/> Экспорт</button>
            <button onClick={() => launchInstance(selectedInstance)} disabled={isLaunching || isRepairing || isBusy} className={`${isRunning ? 'bg-red-500 hover:bg-red-600 text-white' : 'bg-jm-accent hover:bg-jm-accent-light text-black'} px-8 py-3 rounded-xl font-bold transition-transform hover:scale-105 shadow-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100`}>
              {isBusy ? <><Loader2 size={20} className="animate-spin" /> Идёт загрузка… {busyPercent}%</> : isLaunching || isRepairing ? <><Loader2 size={20} className="animate-spin" /> {isRepairing ? "Починка..." : "Запуск..."}</> : isRunning ? <><Square size={20} fill="currentColor" /> СТОП</> : <><Play size={20} fill="currentColor" /> ИГРАТЬ</>}
            </button>
            <button onClick={() => invoke("open_folder", { instanceId: selectedInstance.id })} className="w-12 bg-white/10 hover:bg-white/20 rounded-xl flex items-center justify-center transition-colors" title="Открыть папку сборки">
        ...
        <div className="flex gap-6 h-full min-h-0">
          <div className="w-64 shrink-0 flex flex-col gap-2">
            {[{ id: "content", label: "Контент", icon: <Puzzle size={18} /> }, { id: "worlds", label: "Миры и серверы", icon: <Globe size={18} /> }, { id: "logs", label: "Логи", icon: <Terminal size={18} /> }, { id: "options", label: "Настройки", icon: <Settings size={18} /> }].map(tab => (
              <button key={tab.id} onClick={() => { setInstanceTab(tab.id); if (tab.id === "worlds" && selectedInstance) { invoke("list_worlds", { instanceId: selectedInstance.id }).then((w: any) => setWorldsList(w || [])); invoke("load_servers").then((s: any) => setRecentServers(Array.isArray(s) ? s : [])); invoke("get_last_world").then((lw: any) => setLastWorldGlobal(lw && lw.instance_id ? lw : null)).catch(() => setLastWorldGlobal(null)); } }} className={`flex items-center gap-3 px-5 py-4 rounded-2xl font-bold transition-all ${instanceTab === tab.id ? 'bg-jm-accent text-black shadow-md' : 'bg-jm-card text-gray-400 hover:text-white border border-white/5 hover:border-white/20'}`}>
                {tab.icon} {tab.label}
              </button>
            ))}
            {confirmDeleteId === selectedInstance.id ? (
              <div className="mt-auto flex flex-col gap-2 p-4 rounded-2xl bg-red-500/10 border border-red-500/30">
                <p className="text-sm text-red-400 font-bold text-center">Удалить навсегда?</p>
                <div className="flex gap-2">
                  <button onClick={() => handleDelete(selectedInstance.id)} className="flex-1 py-2 rounded-xl font-bold text-white bg-red-500 hover:bg-red-600 transition-colors text-sm">Да</button>
                  <button onClick={() => setConfirmDeleteId(null)} className="flex-1 py-2 rounded-xl font-bold text-gray-400 bg-white/10 hover:bg-white/20 transition-colors text-sm">Нет</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setConfirmDeleteId(selectedInstance.id)} className="mt-auto flex items-center gap-3 px-5 py-4 rounded-2xl font-bold text-red-500 bg-red-500/10 hover:bg-red-500 hover:text-white transition-colors border border-red-500/20">
                <Trash2 size={18} /> Удалить
              </button>
            )}
          </div>

          <div className="flex-grow bg-jm-card rounded-3xl border border-white/10 shadow-xl p-6 overflow-hidden flex flex-col relative">
            {instanceTab === "content" && (
              <div className="flex flex-col h-full">
                <div className="flex justify-between items-center mb-4">
                  <div className="flex gap-2 bg-black/30 p-1 rounded-xl border border-white/5">
                    {[{ id: "mods", label: "Моды" }, { id: "resourcepacks", label: "Ресурспаки" }, { id: "shaderpacks", label: "Шейдеры" }].map(t => (
                      <button key={t.id} onClick={() => { setContentTab(t.id); loadContent(t.id as "mods" | "resourcepacks" | "shaderpacks"); }} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${contentTab === t.id ? 'bg-jm-accent text-black shadow-md' : 'text-gray-400 hover:text-white'}`}>{t.label}</button>
                    ))}
                  </div>
                ...
                <div className="flex-grow overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-2">
                  {displayedMods.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-gray-500"><Puzzle size={64} className="mb-4 opacity-20" /><p className="text-xl font-bold text-white mb-2">Нет контента</p><p className="text-sm">Нажмите «Добавить», чтобы открыть браузер.</p></div>
                  ) : (
                    displayedMods.map(m => (
                      <div key={m.filename} className={`flex items-center p-3 rounded-xl border transition-colors ${m.enabled ? 'bg-black/30 border-white/5 hover:border-white/10' : 'bg-black/50 border-red-500/20 opacity-60'}`}>
                        ...
                        <div className="cursor-pointer hover:opacity-80 transition-opacity">
                          {m.icon_url ? <img src={m.icon_url} className="w-10 h-10 rounded-lg object-cover bg-black/50 shrink-0" /> : <div className="w-10 h-10 rounded-lg bg-black/50 flex items-center justify-center shrink-0"><Puzzle size={20} className="text-gray-500"/></div>}
                        </div>
                        ...
                        <div className="flex-1 ml-4 min-w-0 grid grid-cols-2 items-center" onClick={() => { if (m.project_id) { setOpenModProjectId(m.project_id); setShowModBrowser(true); } }}>
                          ...
                          <p className="text-sm text-gray-400 truncate">{m.version_name}</p>
                        </div>
                        ...
                        <div className="flex items-center gap-2 ml-4 shrink-0">
                          ...
                          {confirmDeleteMod === m.filename ? (
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={() => setConfirmDeleteMod(null)} className="bg-white/10 hover:bg-white/20 text-gray-400 text-xs px-3 py-2 rounded-lg font-bold transition-colors">Нет</button>
                            </div>
                          ) : (
                            <button onClick={() => setConfirmDeleteMod(m.filename)} className="bg-white/5 hover:bg-red-500/20 text-gray-400 hover:text-red-500 transition-colors p-2 rounded-lg"><Trash2 size={18}/></button>
                          )}
                        </div>
                      </div>
                    ))
                ...
                {lastWorldGlobal && (
                  <div>
                    ...
                    <div className="p-3 rounded-xl bg-jm-accent/10 border border-jm-accent/30 text-white">
                      ...
                      <div className="text-xs text-gray-400 truncate">Сборка: {lastWorldGlobal.instance_name}</div>
                    </div>
                  </div>
                )}
                <div>
                  ...
                  <div className="flex flex-col gap-2 max-h-48 overflow-y-auto custom-scrollbar">
                    {worldsList.length === 0 ? (
                      <p className="text-gray-500 py-4">Нет сохранённых миров</p>
                    ) : (
                      worldsList.map((w) => (
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2 max-h-48 overflow-y-auto custom-scrollbar">
                    {recentServers.length === 0 ? (
                      <p className="text-gray-500 py-4">Нет сохранённых серверов</p>
                    ) : (
                      recentServers.map((srv: any, i: number) => (
                        <button key={i} onClick={() => launchInstance(selectedInstance, srv.ip)} disabled={isLaunching || isRepairing || isBusy} className="flex items-center gap-4 p-3 rounded-xl bg-black/30 border border-white/5 hover:border-jm-accent/50 hover:bg-jm-accent/10 text-left transition-colors disabled:opacity-50">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="text-xs text-gray-400 truncate">{srv.ip}</div>
                          </div>
                          <Play size={16} className="text-jm-accent ml-auto shrink-0" />
                        </button>
                      ))
            ...
            {instanceTab === "logs" && (
              <div className="h-full flex flex-col">
                ...
                <div className="flex-grow bg-[#050805] rounded-xl border border-white/10 p-4 font-mono text-xs text-gray-300 overflow-y-auto custom-scrollbar leading-relaxed">
            ...
            {instanceTab === "options" && (
              <div className="flex flex-col h-full">
                <div className="flex gap-4 mb-6 border-b border-white/10 pb-4">
                  <button onClick={() => setSettingsSubTab("general")} className={`font-bold transition-colors ${settingsSubTab === "general" ? "text-jm-accent" : "text-gray-400 hover:text-white"}`}>Общие</button>
                  {packSourceInfo && (
                    <button onClick={() => setSettingsSubTab("pack")} className={`font-bold transition-colors ${settingsSubTab === "pack" ? "text-jm-accent" : "text-gray-400 hover:text-white"}`}>Сборка</button>
                  )}
                  <button onClick={() => {
                ...
                }} className={`font-bold transition-colors ${settingsSubTab === "core" ? "text-jm-accent" : "text-gray-400 hover:text-white"}`}>Ядро (Core)</button>
                </div>
                ...
                {settingsSubTab === "general" && (
                  <div className="space-y-6">
                    ...
                    <div className={`space-y-6 transition-opacity ${instSettings.override_global ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
                      <div>
                        <label className="text-sm text-gray-400 mb-2 block">Выделение ОЗУ: <strong className="text-jm-accent">{instSettings.ram_mb} MB</strong></label>
                      ...
                      <div>
                        <label className="text-sm text-gray-400 mb-2 block">Аргументы JVM</label>
                    ...
                    <p className="text-xs text-gray-500 -mt-2">Linux: __NV_PRIME_RENDER_OFFLOAD. Windows: настройте GPU в параметрах системы.</p>
                  </div>
                )}
                ...
                {settingsSubTab === "pack" && packSourceInfo && (
                  <div className="space-y-6">
                    <div className="p-4 bg-black/30 rounded-2xl border border-white/5">
                      <h4 className="text-sm font-bold text-gray-300 mb-3">Название сборки</h4>
                    ...
                    {packSourceInfo.source === "modrinth" && packVersions.length > 0 && (
                      <div className="p-4 bg-black/30 rounded-2xl border border-white/5">
                        <h4 className="text-sm font-bold text-gray-300 mb-3">Версия сборки</h4>
                    ...
                    <div className="p-4 bg-red-500/10 rounded-2xl border border-red-500/30">
                      ...
                      <p className="text-xs text-gray-400 mb-3">Сборка перестанет показывать обновления. Файлы не удаляются.</p>
                    ...
                    <div className="flex items-center gap-5 p-5 bg-black/30 rounded-2xl border border-white/5">
                      ...
                      <div>
                        ...
                        <div className="flex gap-3 mt-1">
                          <span className="text-sm text-gray-400">Minecraft <strong className="text-white">{selectedInstance.game_version}</strong></span>
                          {selectedInstance.loader_version && <span className="text-sm text-gray-400">Ядро <strong className="text-jm-accent">{selectedInstance.loader_version}</strong></span>}
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4 p-4 bg-black/20 rounded-2xl border border-white/5">
                      <h4 className="text-sm font-bold text-gray-300 mb-2">Сменить ядро</h4>
            ...
            <AnimatePresence>
              {showModBrowser && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex">
                  <div className="flex-grow bg-black/60 backdrop-blur-sm" onClick={() => { setShowModBrowser(false); setOpenModProjectId(undefined); loadMods(); }} />
                  <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="w-[70vw] h-full bg-jm-bg border-l border-white/10 shadow-2xl flex flex-col">
                    <div className="flex justify-between items-center p-4 bg-jm-card border-b border-white/10 shrink-0">
                      <h3 className="font-bold text-white text-lg">Добавление контента в {selectedInstance.name}</h3>
                      <button onClick={() => { setShowModBrowser(false); setOpenModProjectId(undefined); loadMods(); }} className="text-gray-400 hover:text-white p-2 bg-black/50 rounded-xl transition-colors"><X size={20}/></button>
                    </div>
  ...
  return (
    <div className="flex flex-col w-full max-w-6xl mx-auto h-full">
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        <h2 className="text-3xl font-bold text-jm-accent-light">Ваши сборки</h2>
        <div className="flex items-center gap-4">
          <div className="w-48">
            <CustomSelect label="" value={globalFilter} onChange={setGlobalFilter} options={[{ value: "all", label: "Все загрузчики" }, { value: "vanilla", label: "Vanilla" }, { value: "fabric", label: "Fabric" }, { value: "forge", label: "Forge" }, { value: "neoforge", label: "NeoForge" }, { value: "quilt", label: "Quilt" }]} />
          </div>
          <button onClick={async () => { try { const res = await invoke("import_instance"); showToast(res as string); loadData(); } catch (e) { showToast(`Ошибка импорта: ${e}`); } }} className="bg-white/10 hover:bg-white/20 text-white px-4 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-colors"><PackageOpen size={18}/> Импорт</button>
          <button onClick={() => setIsCreating(true)} className="bg-jm-accent hover:bg-jm-accent-light text-black px-4 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-colors"><Plus size={18}/> Создать сборку</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto custom-scrollbar pb-10">
        {displayedInstances.map((inst, idx) => {
          ...
          return (
            <motion.div key={inst.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05, duration: 0.3 }} whileHover={{ y: -4, boxShadow: "0 10px 30px rgba(134,168,134,0.15)" }} onClick={() => setSelectedInstance(inst)} className="bg-jm-card border border-white/10 rounded-3xl p-5 flex flex-col hover:border-jm-accent transition-all cursor-pointer group relative overflow-hidden">
              <div className="flex items-center gap-4 mb-6 relative z-10">
                {inst.icon ? (
                  <img src={convertFileSrc(inst.icon)} alt="" className="w-16 h-16 rounded-2xl object-cover shadow-inner border border-white/10" onError={e => { (e.target as HTMLImageElement).style.display = "none"; (e.target as HTMLImageElement).nextElementSibling?.classList.remove("hidden"); }} />
                ) : null}
                <div className={`w-16 h-16 rounded-2xl bg-black/50 border border-white/20 flex items-center justify-center text-lg font-medium text-white/70 shrink-0 ${inst.icon ? "hidden" : ""}`}>{inst.name?.charAt(0)?.toUpperCase() || "?"}</div>
                <div><h3 className="text-xl font-bold text-white mb-1 truncate">{inst.name}</h3><div className="flex gap-2"><span className="text-xs bg-white/10 px-2 py-1 rounded-md text-gray-300 capitalize">{inst.loader}</span><span className="text-xs bg-white/10 px-2 py-1 rounded-md text-gray-300">{inst.game_version}</span></div></div>
              </div>
              <div className="mt-auto flex gap-2 relative z-10">
                <button onClick={(e) => { e.stopPropagation(); launchInstance(inst); }} disabled={isLaunching || isRepairing || busyInstanceId === inst.id} className={`flex-1 font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors border disabled:opacity-50 disabled:cursor-not-allowed ${isRunning ? 'bg-red-500/20 text-red-500 border-red-500/30 hover:bg-red-500 hover:text-white' : 'bg-jm-accent/10 text-jm-accent border-jm-accent/30 hover:bg-jm-accent hover:text-black'}`}>
                  {busyInstanceId === inst.id ? <><Loader2 size={16} className="animate-spin" /> Загрузка...</> : isLaunching || isRepairing ? <><Loader2 size={16} className="animate-spin" /> ...</> : isRunning ? <><Square size={16} fill="currentColor" /> СТОП</> : <><Play size={16} fill="currentColor" /> ИГРАТЬ</>}
                </button>
                <button className="w-12 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white rounded-xl flex items-center justify-center transition-colors border border-white/10">
                  <Wrench size={16} />
                </button>
              </div>
            </motion.div>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    {isLoadingVersions ? <div className="h-[72px] flex items-end"><div className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-gray-400 flex items-center gap-2"><Loader2 className="animate-spin" size={16}/> Загрузка...</div></div> : <CustomSelect label="Версия игры" value={newVersion} onChange={setNewVersion} options={availableVersions.map(v => ({ value: v, label: v }))} />}
                  </div>
                  {newLoader !== "vanilla" && <div className="flex-1">
                    {isLoadingLoaderVersions ? <div className="h-[72px] flex items-end"><div className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-gray-400 flex items-center gap-2"><Loader2 className="animate-spin" size={16}/> Загрузка...</div></div> : <CustomSelect label="Версия ядра" value={newLoaderVersion} onChange={setNewLoaderVersion} options={availableLoaderVersions.map(v => ({ value: v, label: v }))} disabled={availableLoaderVersions.length === 0}/>}
                  </div>}
                </div>
              </div>
