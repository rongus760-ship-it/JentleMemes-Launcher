...
export function applyThemeColors(colors: ThemeColors) {
  ...
  // «Сырые» (solid) алиасы — нужны `html.jm-wallpaper-ui` для color-mix без self-reference.
  // Без них режим обоев для custom/auto-bg тем сделал бы --card циклическим и плитки
  // схлопнулись бы в transparent.
  root.style.setProperty("--card-raw", colors.card);
  root.style.setProperty("--text", colors.text);
  root.style.setProperty("--text-secondary", colors.textSecondary);
  root.style.setProperty("--accent-rgb", colors.accentRgb);
  root.style.setProperty("--input-bg", colors.inputBg);
  root.style.setProperty("--input-bg-raw", colors.inputBg);
  root.style.setProperty("--header-bg", colors.headerBg);
  root.style.setProperty("--header-bg-raw", colors.headerBg);
...
export function extractColorsFromImage(src: string): Promise<{ dominant: string; isLight: boolean }> {
  return new Promise((resolve) => {
    const img = new Image();
    /* data:/blob: — не ставим crossOrigin: у WebKit иначе canvas/декод может сорваться → фолбек #86A886. */
...
let _customThemesInvalidatorInstalled = false;

/**
 * Подписка на `settings_updated`, чтобы инвалидировать локальный кеш
 * кастомных тем, когда настройки меняются из другого места (другой таб,
 * onboarding, overlay). Без этого `loadCustomThemes()` возвращал бы устаревший
 * снапшот и в UI всплывал бы баг «созданную тему не видно».
 */
async function ensureCustomThemesInvalidator(): Promise<void> {
  if (_customThemesInvalidatorInstalled) return;
  _customThemesInvalidatorInstalled = true;
  try {
    const { listen } = await import("@tauri-apps/api/event");
    await listen("settings_updated", () => {
      _customThemesCache = null;
    });
  } catch {
    /* ignore — на web-preview слушатель может быть недоступен */
  }
}

export async function loadCustomThemes(): Promise<CustomThemeDef[]> {
  await ensureCustomThemesInvalidator();
...
export async function saveCustomThemes(themes: CustomThemeDef[]) {
  ...
  try {
    // patch_settings атомарно мержит дельту в settings.json и эмитит
    // settings_updated — это устраняет race condition, из-за которого
    // пара load_settings → save_settings могла перезаписать изменения,
    // сделанные в другом месте (например, выбор визуального пресета).
    await invoke("patch_settings", { delta: { custom_themes: themes } });
  } catch (e) {
    console.error("Failed to save custom themes:", e);
    // При ошибке сбрасываем кеш, чтобы UI перечитал реальное состояние.
    _customThemesCache = null;
  }
}
