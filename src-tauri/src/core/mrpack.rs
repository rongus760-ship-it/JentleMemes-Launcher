...
use crate::config::{InstanceConfig, get_data_dir, PackSource, InstalledFile, save_pack_source, load_pack_source};
...
pub const LAUNCHER_VERSION_LABEL: &str = "1.0.5-beta.1";
...
pub async fn export_mrpack_async(app: &AppHandle, id: &str, selected_folders: Vec<String>) -> Result<String> {
    let inst_dir = get_data_dir().join("instances").join(id);
    if !inst_dir.exists() {
...
fn build_jentlepack_manifest(
    ...
    let pack_source = load_pack_source(inst_dir).ok().flatten();
    let servers = read_optional_json(&inst_dir.join("servers.json"));
    let last_world = read_optional_json(&inst_dir.join("last_world.json"));
    ...
    if let Some(ps) = pack_source {
        manifest["packSource"] = serde_json::to_value(&ps)?;
    }
    if let Some(s) = servers {
        manifest["servers"] = s;
    }
    if let Some(w) = last_world {
        manifest["lastWorld"] = w;
    }
    Ok(manifest)
}

fn apply_jentlepack_manifest(inst_dir: &Path, new_instance_id: &str, manifest: &serde_json::Value) -> Result<()> {
    ...
    if let Some(servers) = manifest.get("servers") {
        fs::write(inst_dir.join("servers.json"), serde_json::to_string_pretty(servers)?)?;
    }
    if let Some(lw) = manifest.get("lastWorld") {
        fs::write(inst_dir.join("last_world.json"), serde_json::to_string_pretty(lw)?)?;
    }
...
pub async fn export_jentlepack_async(app: &AppHandle, id: &str, selected_folders: Vec<String>) -> Result<String> {
    let inst_dir = get_data_dir().join("instances").join(id);
    if !inst_dir.exists() {
        return Err(Error::Custom("Сборка не найдена".into()));
    }
...
pub async fn import_instance_packed(app: &AppHandle) -> Result<ImportInstanceResult> {
    ...
    let has_jentle = tokio::task::spawn_blocking(move || extract_import_archive(&zip_path, &inst_dir_clone))
        .await
    ...
    if has_jentle {
        ...
        let manifest: serde_json::Value = serde_json::from_str(&manifest_str).map_err(|e| {
            Error::Custom(format!("Некорректный jentlepack.json: {}", e))
        })?;
