...
/// Не тащим в архив локальные JRE, кэш лаунчера, мету обновлений и т.п.
pub fn export_skip_path(rel: &str) -> bool {
    let rel = rel.trim_start_matches('/').trim_end_matches('/');
    if rel.is_empty() {
        return false;
    }
    if rel.eq_ignore_ascii_case("pack_source.json") {
        return true;
    }
    if rel == "launcher-console.log" || rel.ends_with("/launcher-console.log") {
        return true;
    }
    let skip_seg = |seg: &str| {
        matches!(
            seg,
            "java"
                | "jre"
...
fn strip_machine_jvm_flags(jvm_args: &str) -> String {
    jvm_args
        .split_whitespace()
        .filter(|t| {
            if t.contains("=/usr/") || t.contains("=/opt/") {
                return false;
            }
            if t.contains("=C:\\") || t.contains("=c:\\") {
                return false;
            }
            if t.contains("=/home/") && t.starts_with("-D") {
                return false;
            }
            true
        })
...
/// Чистим `instance.json` для переноса: без локальных путей, времени игры и привязки к источнику обновлений.
pub fn export_instance_json_for_transfer(raw_json: &str) -> Result<String> {
    let mut conf: crate::config::InstanceConfig = serde_json::from_str(raw_json)
        .map_err(|e| Error::Custom(format!("instance.json: некорректный JSON ({e})")))?;
    if let Some(ref mut s) = conf.settings {
        s.custom_java_path.clear();
        s.jvm_args = strip_machine_jvm_flags(&s.jvm_args);
    }
    conf.playtime = 0;
    if conf.icon.contains('/') || conf.icon.contains('\\') {
        conf.icon.clear();
    }
    serde_json::to_string_pretty(&conf).map_err(|e| Error::Custom(e.to_string()))
}
...
/// Ключ зависимости в `modrinth.index.json` (Modrinth: `fabric-loader`, не `fabric`).
fn modrinth_pack_dependency_key(loader: &str) -> String {
    match loader {
...
fn index_skeleton(inst_dir: &Path, id: &str) -> Result<Value> {
    ...
    if loader != "vanilla" && !loader_version.is_empty() {
        deps.insert(
            modrinth_pack_dependency_key(loader),
            Value::String(loader_version.to_string()),
        );
    }
...
pub async fn build_export_index_and_overrides(
    ...
    // instance.json — без путей к локальной Java (временный файл)
    let ij = inst_dir.join("instance.json");
    if ij.exists() {
        let raw = fs::read_to_string(&ij)?;
        let sanitized = export_instance_json_for_transfer(&raw)?;
        let tmp = std::env::temp_dir().join(format!(
            "jm_export_inst_{}_{}.json",
            pack_id,
            uuid::Uuid::new_v4()
        ));
        fs::write(&tmp, sanitized)?;
        override_entries.push(("instance.json".into(), tmp));
    }
