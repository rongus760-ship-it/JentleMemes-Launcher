//! Discord Rich Presence через локальный IPC. Нужен **Application ID** из Discord Developer Portal.
//! Пока ID пустой — вызовы безопасно игнорируются.
...
/// Укажите Application ID (не секрет) своего приложения: https://discord.com/developers/applications
/// После создания приложения скопируйте «Application ID» сюда. Пустая строка — Rich Presence отключён на уровне клиента.
// Если запускаете лаунчер из корня репозитория — вставьте сюда тот же ID, что в Discord Portal.
// В копии JentleMemes-Launcher/ этот же файл может уже содержать ID.
pub const DISCORD_APPLICATION_ID: &str = "";
...
pub fn set_playing_minecraft(
    ...
    if let Some(ip) = server_ip {
        if !ip.is_empty() {
            state = format!("{} · {}", truncate(&state), truncate(&format!("Сервер: {}", ip)));
        }
    }
