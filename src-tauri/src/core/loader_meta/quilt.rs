...
const GAME_URL: &str = "https://meta.quiltmc.org/v3/versions/game";

pub async fn game_versions(include_snapshots: bool) -> Result<Vec<String>> {
    let body = fetch_text_cached("quilt:game", GAME_URL).await?;
    let res: Value = serde_json::from_str(&body)
        .map_err(|e| Error::Custom(format!("Quilt game meta JSON: {e}")))?;
    let arr = res
        .as_array()
        .ok_or_else(|| Error::Custom("Quilt meta: ожидался массив".into()))?;
